function [P_tX,P_Xt,pthat,features] = naivebayesdecoder_simpler(tensor,opt)
    ...
    for kk = 1 : opt.test.n_trials
        ...
        for tt = test_time_idcs
            ...
            % normalize observations using condition-specific statistics
            if opt.normalize
                x = (x - x_mus(:,condition_set == opt.condition(test_idx))); % ./ ...
%                     x_sigs(:,condition_set == opt.condition(test_idx));
            end
...
%             p_tX = p_t .* prod(log(p_tx(~nan_flags,:) + 1),1)';
            p_tX = p_t .* prod(p_tx(~nan_flags,:) * n_timepoints + fudge,1)';
            if all(isnan(p_tX)) || any(isinf(p_tX)) || all(p_tX == 0)
                a=1
            end
            p_X = nansum(p_tX);
            P_tX(tt,:,kk) = p_tX / p_X;
